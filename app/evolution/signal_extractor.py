...
class SignalExtractor:
    ...
    _ENGLISH_PREFERENCE_PATTERNS = (
        (re.compile(r"^\s*i\s+(?:really\s+|still\s+|just\s+)?like\s+(?P<object>.+)$", re.I), "likes"),
        (re.compile(r"^\s*i\s+(?:really\s+)?love\s+(?P<object>.+)$", re.I), "likes"),
        (re.compile(r"^\s*i\s+(?:really\s+)?enjoy\s+(?P<object>.+)$", re.I), "likes"),
        (re.compile(r"^\s*i\s+(?:really\s+)?dislike\s+(?P<object>.+)$", re.I), "dislikes"),
        ...
        (re.compile(r"^\s*i\s+(?:mostly\s+|usually\s+|often\s+)?use\s+(?P<object>.+)$", re.I), "uses"),
        (re.compile(r"^\s*i\s+(?:mostly\s+|usually\s+|often\s+)?code\s+in\s+(?P<object>.+)$", re.I), "uses"),
        (re.compile(r"^\s*i\s+(?:mostly\s+|usually\s+|often\s+)?work\s+with\s+(?P<object>.+)$", re.I), "uses"),
    )
    _CHINESE_PREFERENCE_PATTERNS = (
        (re.compile(r"^\s*\u6211(?:\u771f\u7684|\u5176\u5b9e)?(?:\u5f88|\u66f4)?\u4e0d\u559c\u6b22(?P<object>.+)$"), "dislikes"),
        (re.compile(r"^\s*\u6211(?:\u771f\u7684|\u5176\u5b9e)?\u8ba8\u538c(?P<object>.+)$"), "dislikes"),
        (re.compile(r"^\s*\u6211(?:\u771f\u7684|\u5176\u5b9e)?(?:\u66f4)?\u504f\u597d(?P<object>.+)$"), "prefers"),
        (re.compile(r"^\s*\u6211(?:\u771f\u7684|\u5176\u5b9e)?(?:\u66f4|\u5f88)?\u559c\u6b22(?P<object>.+)$"), "likes"),
        (re.compile(r"^\s*\u6211(?:\u5e73\u65f6|\u4e00\u822c|\u901a\u5e38|\u7ecf\u5e38|\u4e3b\u8981)?(?:\u90fd)?(?:\u7528|\u4f7f\u7528)(?P<object>.+)$"), "uses"),
        (re.compile(r"^\s*\u6211(?:\u5e73\u65f6|\u4e00\u822c|\u901a\u5e38|\u7ecf\u5e38|\u4e3b\u8981)?(?:\u90fd)?\u7528(?P<object>.+)\u5199\u4ee3\u7801$"), "uses"),
    )
    _COPY_MARKERS = (
        "copied",
        "copy",
    ...
    async def _extract_explicit_preference_lesson(self, event: Event) -> Lesson | None:
        ...
        review = await self.review_preference_candidate(
            text=text,
            candidate=candidate,
            default_classification="quoted_or_forwarded" if quoted_candidate is not None else "self_reported",
        )
        return self._build_reviewed_preference_lesson(event, candidate=candidate, review=review)
    ...
    async def _extract_implicit_preference_lesson(self, event: Event) -> Lesson | None:
        ...
        object_value = self._normalize_preference_object(str(extracted.get("object", "")).strip())
        ...
        memory_tier = "session_hint" if durability == "situational" else "inference_candidate"
        return Lesson(
            ...
            details={
                ...
                "review_reason": (
                    ...
                    else ("ambiguous_preference_evidence" if classification == "uncertain" else "")
                ),
    ...
    def _quoted_or_copied_preference_candidate(cls, text: str) -> dict[str, str] | None:
        ...
        if not any(marker in lowered or marker in normalized for marker in cls._COPY_MARKERS):
            return None
        segments: list[str] = []
        for pattern in cls._QUOTE_SEGMENT_PATTERNS:
            segments.extend(match.group("segment").strip() for match in pattern.finditer(normalized))
        for separator in (":", "\uff1a", "-", "\u2014"):
    ...
    async def _extract_implicit_preference_candidate(self, text: str) -> dict[str, Any] | None:
        ...
        try:
            ...
            if self.circuit_breaker is None:
                ...
                response = await self.circuit_breaker.call(self._IMPLICIT_EVENT_KEY, generate, payload)
    ...
    def _build_reviewed_preference_lesson(
        ...
        *,
        ...
        review_confidence = float(review.get("confidence", 0.58))
        ...
        review_reason = "quoted_or_copied_content" if classification == "quoted_or_forwarded" else "ambiguous_preference_evidence"
        return cls._build_preference_lesson(
            event,
            candidate=candidate,
            explicit_user_statement=False,
    ...
    async def review_preference_candidate(
        ...
        *,
        ...
        fallback = self._fallback_review(text=text, candidate=candidate, classification=default_classification)
        if self.model_registry is None:
            return fallback
        try:
            generate = self.model_registry.chat(self._REVIEWER_PROFILE).generate
            ...
            if self.circuit_breaker is None:
                ...
                response = await self.circuit_breaker.call(self._REVIEWER_EVENT_KEY, generate, payload)
        except (ProviderRequestError, KeyError, NotImplementedError, ValueError):
            return fallback
    ...
    @classmethod
    def _fallback_review(cls, *, text: str, candidate: dict[str, str], classification: str) -> dict[str, Any]:
